                                ...
                                <tr class="{{ $index % 2 === 0 ? 'bg-gray-100' : 'bg-white' }}">
                                    ...
                                    <td class="px-2 py-2 whitespace-nowrap border border-gray-200 text-center">
                                        @if($log->action != 'delete')
                                            <form action="{{ route('admin.undo', $log->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="px-3 py-1 text-sm font-medium leading-5 text-white bg-red-500 rounded-md hover:bg-red-600 focus:outline-none focus:bg-red-600">
                                                    {{ __('Undo') }}
                                                </button>
                                            </form>
                                        @endif
